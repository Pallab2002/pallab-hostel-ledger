...
if st.session_state.get("opening_open"):
    with st.form("opening_form"):
        ...
        st.caption("Enter the opening balance manually. No previous-month option.")
        o1,o2 = st.columns(2)
        with o1:
            om = st.number_input("🍛 Mess Opening Balance (₹)", min_value=0.0,
                                 value=float(month["mess_opening"]), step=1.0)
        with o2:
            oc = st.number_input("☕ Canteen Opening Balance (₹)", min_value=0.0,
                                 value=float(month["canteen_opening"]), step=1.0)
        b1,b2 = st.columns(2)
        with b1:
            save = st.form_submit_button("Save Opening Balance", type="primary", use_container_width=True)
        with b2:
...
if st.session_state.get("meal_open"):
    ...
    with st.form("meal_form"):
        ...
        b1,b2 = st.columns(2)
        with b1:
            save = st.form_submit_button("Save Meal", type="primary", use_container_width=True)
        with b2:
            cancel = st.form_submit_button("Cancel", use_container_width=True)
        if save:
...
if st.session_state.get("money_open"):
    ...
    with st.form("money_form"):
        st.markdown(f"### 💳 Monthly Money — {month_label(y,mo)}")
        st.caption("New entries start with today's live date. Money belongs only to the selected month.")
        f1,f2 = st.columns(2)
        with f1:
        ...
        b1,b2 = st.columns(2)
        with b1:
            save = st.form_submit_button("Add Money",type="primary",use_container_width=True)
        with b2:
            cancel = st.form_submit_button("Cancel",use_container_width=True)
        if save:
            if amount <= 0:
...
st.markdown(
    ...
    "Pallab's Hostel Ledger · Supabase PostgreSQL · Ultra Fast</div>",
    unsafe_allow_html=True
)
