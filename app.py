...
# ============================================================
# Pallab's Hostel Ledger — Streamlit + Supabase PostgreSQL
# Shared Supabase database version of the HTML tracker.
# ============================================================
...
# -----------------------------
def db():
    """Connect to Supabase using the Session Pooler URL stored in Streamlit Secrets."""
    try:
        db_url = st.secrets["SUPABASE_DB_URL"]
    except Exception:
        st.error(
            "Supabase connection is not configured. Add SUPABASE_DB_URL "
...
def ensure_month(month_key: str):
    ...
    try:
        with con.cursor() as cur:
            cur.execute(
                "INSERT INTO month_settings(month) VALUES (%s) "
...
def get_month(month_key):
    ...
    try:
        with con.cursor() as cur:
            cur.execute(
                "SELECT * FROM month_settings WHERE month=%s", (month_key,)
            )
            return cur.fetchone()
    finally:
        con.close()
...
def get_meals(month_key):
    ...
    try:
        with con.cursor() as cur:
            cur.execute(
                "SELECT * FROM meals WHERE TO_CHAR(date,'YYYY-MM')=%s ORDER BY date",
                (month_key,)
            )
            return cur.fetchall()
    finally:
...
def get_meal(ds):
    ...
    try:
        with con.cursor() as cur:
            cur.execute("SELECT * FROM meals WHERE date=%s", (ds,))
            return cur.fetchone()
    finally:
        con.close()
...
def add_deposit(ds, account, amount, note):
    ...
    try:
        with con.cursor() as cur:
            cur.execute(
                "INSERT INTO deposits(date,account,amount,note) VALUES(%s,%s,%s,%s)",
...
def seed_imported():
    """Seed the original v20 meal history only if the Supabase meals table is empty."""
    con = db()
    try:
        with con.cursor() as cur:
            ...
            if count == 0:
                for x in IMPORTED_MEALS:
                    ...
                    """, (
                        x["date"], bool(x["mm"]), bool(x["mn"]),
                        bool(x["cm"]), bool(x["ce"]), x.get("note","")
                    ))
        con.commit()
    finally:
        con.close()
...
@media (min-width: 1100px) {
  .block-container { max-width: 1240px; padding-left: 1.4rem; padding-right: 1.4rem; }
...
@media (max-width: 700px) {
  .block-container {
    padding: .65rem .55rem 1.3rem !important;
  }
  .hero {
    border-radius: 19px;
    padding: 16px 15px;
  ...
  .account-mess, .account-cant { margin-bottom: 8px; }
  ...
  .metric-label { font-size: 9px; line-height: 1.25; }
  .metric-value { font-size: 20px; }

  .card { padding: 13px; border-radius: 16px; }
  .section-title { font-size: 16px; }
  .balance { font-size: 27px; }
  ...
  /* Calendar stays 7 columns, but becomes compact and touch-friendly */
  .calendar { gap: 3px; }
  .dow { font-size: 8px; padding: 3px 0; }
  .day {
  ...
  /* Deposit rows can scroll horizontally rather than breaking the page */
...
st.markdown("<div style='text-align:center;color:#687287;font-size:10px;padding:22px 0 4px'>Pallab's Hostel Ledger · Shared Supabase database · meal counts only</div>", unsafe_allow_html=True)
